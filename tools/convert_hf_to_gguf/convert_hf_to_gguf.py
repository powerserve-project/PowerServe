...
class Model:
        ...
        for name, data_torch in self.get_tensors():
            ...
            for new_name, data in ((n, d.squeeze().numpy()) for n, d in self.modify_tensors(data_torch, name, bid)):
                ...
                if data_qtype is False and any(
                    self.match_model_tensor_name(new_name, key, bid)
                    for key in (gguf.MODEL_TENSOR.TOKEN_EMBD, gguf.MODEL_TENSOR.OUTPUT)
                ):
    ...
    def does_token_look_special(self, token: str | bytes) -> bool:
        ...
        seems_special = token_text in ("<pad>", "<mask>", "<2mass>", "[@BOS@]")  # deepseek-coder  # gemma{,-2}
            ...
            if rope_scaling.get("rope_type", "").lower() == "llama3":
                ...
                if not self.is_lora:
                    self.gguf_writer.add_tensor(
                        self.format_tensor_name(gguf.MODEL_TENSOR.ROPE_FREQS), np.array(rope_factors, dtype=np.float32)
                    )
...
class MiniCPM3Model(Model):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_tensor(
            gguf.TENSOR_NAMES[gguf.MODEL_TENSOR.ROPE_FACTORS_LONG] + ".weight", np.array(long_factors, dtype=np.float32)
        )
        self.gguf_writer.add_tensor(
...
class LazyTorchTensor(gguf.LazyBase):
    ...
    _dtype_map: dict[torch.dtype, type] = {torch.float16: np.float16, torch.float32: np.float32}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="only print out a split plan and exit, without writing any new files"
    )
    parser.add_argument(
