...
def dump_metadata_json(reader: GGUFReader, args: argparse.Namespace) -> None:
    ...
    result = {"filename": args.model, "endian": file_endian, "metadata": metadata, "tensors": tensors}
...
def markdown_table_with_alignment_support(header_map: list[dict[str, str]], data: list[dict[str, Any]]):
    ...
    rows.append(
        "|".join(
            strAlign(rowsPadding[index], columnEntry.get("align"), str(columnEntry["header_name"]))
            for index, columnEntry in enumerate(header_map)
        )
    )
    rows.append(
...
def dump_markdown_metadata(reader: GGUFReader, args: argparse.Namespace) -> None:
            ...
            if field.types[0] == GGUFValueType.ARRAY:
                ...
                if curr_type == GGUFValueType.STRING:
                    ...
                    for element_pos in range(render_element):
                        truncate_length = 30
                        value_string = str(
                            bytes(field.parts[-1 - (total_elements - element_pos - 1) * 2]), encoding="utf-8"
                        )
                        if len(value_string) > truncate_length:
                            head = escape_markdown_inline_code(value_string[: truncate_length // 2])
                            tail = escape_markdown_inline_code(value_string[-truncate_length // 2 :])
    ...
    if not args.no_tensors:
        ...
        for key, tensor in enumerate(reader.tensors):
            ...
            tensor_mapping_table.append({
                "t_id": key,
                "layer_name": tensor.name,
                "data_offset": data_offset_pretty,
                "data_size": data_size_pretty,
            })
        ...
        markdown_content += markdown_table_with_alignment_support(
            tensors_mapping_table_header_map, tensor_mapping_table
        )
        markdown_content += "\n"
        ...
        for group in tensor_prefix_order:
            ...
            for tensor in tensors:
                prettify_element_est_count_size = max(
                    prettify_element_est_count_size, len(str(element_count_rounded_notation(tensor.n_elements)))
                )
            ...
            for tensor in tensors:
                human_friendly_name = translate_tensor_name(
                    tensor.name.replace(".weight", ".(W)").replace(".bias", ".(B)")
                )
                pretty_dimension = " x ".join(
                ...
                tensor_dump_table.append({
                    "t_id": tensor_name_to_key[tensor.name],
                    "layer_name": tensor.name,
                    "human_layer_name": human_friendly_name,
                    "element_count": element_count_string,
                    "pretty_dimension": pretty_dimension,
                    "tensor_type": type_name_string,
                })
