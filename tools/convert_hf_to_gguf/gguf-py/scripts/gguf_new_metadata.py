...
def main() -> None:
    tokenizer_metadata = (
        getattr(gguf.Keys.Tokenizer, n) for n in gguf.Keys.Tokenizer.__dict__.keys() if not n.startswith("_")
    )
    token_names = dict(
        (n.split(".")[-1][: -len("_token_id")], n) for n in tokenizer_metadata if n.endswith("_token_id")
    )
    ...
    parser.add_argument(
        "--general-description", type=str, help="The models general.description", metavar='"Description ..."'
    )
    parser.add_argument(
        "--chat-template",
        type=str,
